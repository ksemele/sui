...
mod tests {
    ...
    /// Create an indexer with latest_checkpoint=LATEST_CHECKPOINT, an optional watermark and
    /// first_checkpoint, add a concurrent pipeline with the given config, and return
    /// the indexer.
    ...
    #[tokio::test]
    async fn test_latest_checkpoint_with_pruner_sets_min_checkpoint_hi() {
    ...
    #[tokio::test]
    async fn test_latest_checkpoint_without_pruner_falls_back_to_genesis() {
    ...
    #[tokio::test]
    async fn test_latest_checkpoint_watermark_takes_priority() {
        let retention = LATEST_CHECKPOINT - 1;
    ...
    #[tokio::test]
    async fn test_first_checkpoint_takes_priority_over_latest_checkpoint() {
        let retention = LATEST_CHECKPOINT - 1;
    ...
    #[tokio::test]
    async fn test_retention_exceeds_latest_checkpoint() {
        let retention = LATEST_CHECKPOINT + 1;
        let indexer = test_next_checkpoint(None, None, pruner_config(retention)).await;
        assert_eq!(indexer.next_checkpoint, 0);
    }
