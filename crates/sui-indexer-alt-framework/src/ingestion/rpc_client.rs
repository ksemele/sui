...
use crate::ingestion::byte_count::ByteCount;
...
impl IngestionClientTrait for RpcClient {
    ...
    async fn checkpoint(&self, checkpoint: u64) -> CheckpointResult {
        ...
        let num_bytes = response
            .extensions()
            .get::<ByteCount>()
            .map_or(0, |c| c.value()) as u64;
        let response = response.into_inner();
        let checkpoint = Checkpoint::try_from(response.checkpoint())
            .map_err(|e| CheckpointError::DecodeError(ProtoConversion(e)))?;
